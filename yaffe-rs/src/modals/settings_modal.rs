...
use crate::{font::*, ui::*, colors::*, widgets::get_drawable_text, YaffeState, Actions, V2};
use crate::modals::*;
...
impl ModalContent for SettingsModal {
    ...
    fn render(&self, settings: &crate::settings::SettingsFile, rect: Rectangle, piet: &mut Graphics2D) {
        ...
        let label = get_drawable_text(FONT_SIZE, "Run At Startup");
        piet.draw_text(*pos, get_font_color(settings), &label); 

        let min = V2::new(pos.x + LABEL_SIZE, pos.y);
        let max = V2::new(pos.x + LABEL_SIZE + 24., pos.y + 24.);
        ...
        piet.draw_rectangle(checkbox, change_brightness(&base, factor));
