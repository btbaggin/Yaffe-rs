use winapi::shared::minwindef::{BYTE, DWORD, FALSE, WORD};
...
use winapi::um::mmdeviceapi::{eConsole, eRender, IMMDevice, IMMDeviceEnumerator, MMDeviceEnumerator};
...
impl Drop for ComString {
    fn drop(&mut self) {
        unsafe {
            winapi::um::oleauto::SysFreeString(self.string);
        }
    }
}
impl Deref for ComString {
    type Target = BSTR;
    fn deref(&self) -> &BSTR { &self.string }
}
...
impl<T: Interface> ComPtr<T> {
    fn default() -> ComPtr<T> { ComPtr { ptr: std::ptr::null_mut() } }
    fn as_mut(&mut self) -> *mut *mut T { &mut self.ptr as *mut *mut T }
    fn is_null(&self) -> bool { self.ptr.is_null() }
}
impl<T: Interface> Deref for ComPtr<T> {
    type Target = T;
    fn deref(&self) -> &T { unsafe { &mut *self.ptr } }
}
...
pub(super) fn get_run_at_startup(task: &str) -> StartupResult<bool> {
    ...
    safe_com_call!(CoCreateInstance(
    ...
    unsafe {
        p_folder.GetTask(*ComString::new(task), p_task.as_mut());
    }
...
pub(super) fn set_run_at_startup(task: &str, value: bool) -> StartupResult<()> {
    ...
    if !value {
        ...
        safe_com_call!(
            p_action.QueryInterface(&IExecAction::uuidof(), p_exec_action.as_mut() as *mut *mut std::os::raw::c_void)
        )?;

        safe_com_call!(p_exec_action.put_Path(*ComString::new(&path.as_os_str().to_string_lossy())))?;
        safe_com_call!(
            p_exec_action.put_WorkingDirectory(*ComString::new(&working_path.as_os_str().to_string_lossy()))
        )?;
        ...
        safe_com_call!(p_folder.RegisterTaskDefinition(
            *ComString::new(task),
        ...
        unsafe {
            winapi::um::oleauto::SysFreeString(*group.n1.n2_mut().n3.bstrVal());
        }
        unsafe {
            winapi::um::oleauto::SysFreeString(*s.n1.n2_mut().n3.bstrVal());
        }
    }
...
pub(super) fn shutdown() -> ShutdownResult {
    ...
    unsafe {
        ...
        let security_name: Vec<u16> = std::ffi::OsStr::new(SE_SHUTDOWN_NAME).encode_wide().chain(once(0)).collect();
...
fn load<S: AsRef<str>>(s: S) -> Option<WindowsInput> {
    ...
    unsafe {
        ...
        if !get_state_ptr.is_null() {
            opt_xinput_get_state = Some(::std::mem::transmute::<
                *mut winapi::shared::minwindef::__some_function,
                unsafe extern "system" fn(u32, *mut crate::os::os_impl::XInputGamepadEx) -> u32,
            >(get_state_ptr));
        }
    }
    ...
    Some(WindowsInput {
        get_state: opt_xinput_get_state.unwrap(),
...
impl crate::input::PlatformGamepad for WindowsInput {
    ...
    fn get_gamepad(&mut self) -> Vec<super::ControllerInput> {
        ...
        if (now - self.last_button_time).as_millis() > 100 {
            ...
            if is_pressed(self, XINPUT_GAMEPAD_START) {
                result.push(super::ControllerInput::ButtonStart);
            }
            if is_pressed(self, XINPUT_GAMEPAD_BACK) {
                result.push(super::ControllerInput::ButtonBack);
            }
            if is_pressed(self, CONTROLLER_GUIDE) {
                result.push(super::ControllerInput::ButtonGuide);
            }
            if is_pressed(self, XINPUT_GAMEPAD_A) {
                result.push(super::ControllerInput::ButtonSouth);
            }
            if is_pressed(self, XINPUT_GAMEPAD_B) {
                result.push(super::ControllerInput::ButtonEast);
            }
            if is_pressed(self, XINPUT_GAMEPAD_X) {
                result.push(super::ControllerInput::ButtonWest);
            }
            if is_pressed(self, XINPUT_GAMEPAD_Y) {
                result.push(super::ControllerInput::ButtonNorth);
            }
            if result.len() > count {
        ...
        {
            let count = result.len();
            if x < 0 && i32::abs(x) > i32::abs(y) {
                result.push(super::ControllerInput::DirectionLeft);
            }
            if y > 0 && i32::abs(y) > i32::abs(x) {
                result.push(super::ControllerInput::DirectionUp);
            }
            if y < 0 && i32::abs(y) > i32::abs(x) {
                result.push(super::ControllerInput::DirectionDown);
            }
            if x > 0 && i32::abs(x) > i32::abs(y) {
                result.push(super::ControllerInput::DirectionRight);
            }
            if result.len() > count {
...
pub(super) fn get_and_update_volume(delta: f32) -> VolumeResult<f32> {
    ...
    safe_com_call!(p_default.Activate(
