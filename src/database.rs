...
static QS_GET_ALL_APPLICATIONS: &str = "SELECT ID, Name, Path FROM Applications";
...
pub(super) fn get_all_platforms() -> Vec<Platform> {
    ...
    result.push(Platform::application(String::from("Applications"), PlatformType::App));
...
pub(super) fn get_all_applications() -> Vec<Executable> {
    crate::log_function!();

    let con = YaffeConnection::new();
    let stmt = create_statement!(con, QS_GET_ALL_APPLICATIONS, );
    ...
    execute_select(stmt, |r| {
        ...
        let (boxart, banner) = crate::assets::get_asset_slot("Application", &name);
        result.push(Executable::new_application(path, name.clone(), boxart, banner));
    });
